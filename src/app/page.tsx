  ...
  return (
    <div className="app-background">
      <main className="app-shell">
        ...
        <nav className="bottom-nav" aria-label="Main navigation">{[{ label: "Home", icon: House }, { label: "My Bookings", icon: CalendarDays }, { label: "Settings", icon: Settings }].map((item) => { const NavIcon = item.icon; return <button key={item.label} className={activeTab === item.label ? "nav-item active" : "nav-item"} onClick={() => { setActiveTab(item.label); if (item.label === "My Bookings") router.push("/bookings"); }}><NavIcon aria-hidden="true" /><small>{item.label}</small></button>; })}</nav>
      </main>
